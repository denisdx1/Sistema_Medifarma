...
class AdminProductosController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            // Query para productos en estado ESPERA (solicitud = 0)
            $query = DB::connection('sqlsrv')
                ...
                ->select([
                    ...
                    'm.idMercado',
                    's.solicitud as estadoSolicitud',
                    'e.estado as estadoGeneral',
                    'c.fechaRegistro as fechaSolicitud',
                    'c.idConfiguracion'
                ])
                ->where('s.solicitud', 'ESPERA') // Solo productos en estado ESPERA
            ...
            Log::error('Error al cargar productos pendientes de aprobación', [
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine()
            ]);
            ...
            return redirect()->back()
                ->with('error', 'Error al cargar productos pendientes: ' . $e->getMessage());
        }
    }
    ...
    public function aprobar(Request $request)
    {
        ...
        try {
            ...
            if (!$restoMarket) {
                ...
                Log::info('Mercado RESTO creado', ['idMercado' => $restoMarketId]);
            } else {
            ...
            Log::info('Producto aprobado - SP ejecutado y mercado cambiado a RESTO', [
                'codigoPresentacion' => $validated['codigoPresentacion'],
                'idConfiguracion' => $validated['idConfiguracion'],
                'cambios' => [
                    'antes' => [
                        'mercado' => $configuracionAntes->mercadoAnterior,
                        'solicitud' => $configuracionAntes->solicitudAnterior,
                        'estado' => $configuracionAntes->estadoAnterior
                    ],
                    'despues' => [
                        'mercado' => $configuracionDespues->mercadoNuevo ?? 'Error al obtener',
                        'solicitud' => $configuracionDespues->solicitudNueva ?? 'Error al obtener',
                        'estado' => $configuracionDespues->estadoNuevo ?? 'Error al obtener'
                    ]
                ],
                'acciones_realizadas' => [
                    'sp_ejecutado' => true,
                    'mercado_cambiado_a_resto' => true,
                    'mercado_resto_id' => $restoMarketId
                ],
            ...
            return response()->json([
                ...
                'detalles' => [
                    ...
                    'solicitudNueva' => $configuracionDespues->solicitudNueva ?? 'Verificar en logs',
                    'estadoAnterior' => $configuracionAntes->estadoAnterior,
                    'estadoNuevo' => $configuracionDespues->estadoNuevo ?? 'Verificar en logs'
                ]
            ]);
            ...
            Log::error('Error al aprobar remoción de producto', [
                'codigoPresentacion' => $validated['codigoPresentacion'],
                'idConfiguracion' => $validated['idConfiguracion'],
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
    ...
    public function denegar(Request $request)
    {
        ...
        try {
            ...
            // Log de auditoría
            Log::info('Solicitud de remoción de producto denegada', [
                'codigoPresentacion' => $validated['codigoPresentacion'],
                'idConfiguracion' => $validated['idConfiguracion'],
            ...
            Log::error('Error al denegar remoción de producto', [
                'codigoPresentacion' => $validated['codigoPresentacion'],
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine()
