            ...
            <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100 hover:shadow-lg transition-shadow duration-200">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-red-600">{{ $estadisticas['administradores'] }}</p>
                    </div>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <form method="POST" action="{{ route('usuarios.toggle-estado', $usuario->idUsuario) }}" class="inline">
                                            @csrf
                                            <button type="submit" 
                                                    ...
                                                    onclick="return confirm('¿Está seguro de {{ $usuario->idEstado == 1 ? 'desactivar' : 'activar' }} este usuario?')">
                                        ...
                                        <button onclick="eliminarUsuario({{ $usuario->idUsuario }}, '{{ $usuario->usuario }}')"
                            ...
                            <div class="border border-gray-300 rounded-lg max-h-40 overflow-y-auto bg-white">
                                @foreach($franquicias as $idFranquicia => $nombreFranquicia)
                                    <label class="flex items-center p-3 hover:bg-gray-50 cursor-pointer border-b border-gray-100 last:border-b-0">
                                        ...
                                        <span class="text-sm text-gray-700">{{ $nombreFranquicia }}</span>
                                    </label>
                                @endforeach
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-40 overflow-y-auto border border-gray-300 rounded-lg p-4">
                        @foreach($franquicias as $idFranquicia => $nombreFranquicia)
                            <label class="flex items-center space-x-2 text-sm">
                                ...
                                <span class="text-gray-700">{{ $nombreFranquicia }}</span>
                            </label>
                        @endforeach
                    </div>
                    <div class="text-red-500 text-sm mt-1 hidden" id="error_crear_idFranquicias"></div>
